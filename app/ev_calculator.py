...
# ユーザー自身の資金管理として独立させ、予想・投票プラン・集計・検証には
# 一切影響させない(のんの要望により追加)。
FIXED_STAKING_BANKROLL = 1_000_000
...
# 2026-09-08: line-boost-sweep-v2(先頭→番手だけ20倍)は、わずか59件という
# 狭いサブグループへの対数尤度最適化であり、過学習の懸念があった。
# その後line-position-matrix診断で、番手→先頭・先頭→番手・先頭→3番手・
# 番手→3番手の4区分(合計175件の実現・機会確率合計約108)を横断して見たところ、
# いずれも1.44〜1.76倍という近い範囲に収まっており、「先頭→番手だけが特別」
