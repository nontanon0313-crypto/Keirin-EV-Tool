...
def refresh_odds_now(race_id: int, db: Session = Depends(get_db)):
    ...
    従来のバッチ取り込み(Termux側スクレイパーが取得→/scraper-import/raceに送信)とは別経路で、
    このエンドポイント自身がoddspark.comへ直接アクセスする。Termux側のスクレイパー
    (scraper/keirin_oddspark_scraper.py)の券種別オッズ取得関数をそのまま流用し、
    実装を二重に持たないようにしている。
    """
    ...
    ref_parts = (race.external_ref or "").split(":")
    if len(ref_parts) != 4 or ref_parts[0] != "oddspark":
        raise HTTPException(status_code=400, detail="このレースはoddspark由来のデータではないため、その場でのオッズ再取得はできません")
    _, jo_code, kaisai_bi, race_no_str = ref_parts
    try:
        race_no = int(race_no_str)
    except ValueError:
        raise HTTPException(status_code=400, detail=f"external_refのレース番号が不正です: {race_no_str}")

    # 遅延importにしてある(起動時の依存読み込みを増やさないため。Render無料プランのメモリ対策)
    from scraper.keirin_oddspark_scraper import parse_odds_simple, parse_odds_axis_based, BET_TYPES
