...
特に次を必ず考慮してください(欠けている場合はその旨を織り込む):
- ライン構成(先行・番手の関係。同ラインは連携しやすく、先頭が残ると番手も上位に来やすい)
- 決まり手の傾向(逃・捲・差・マの回数)
- S/H/B回数、脚質、競走得点、直近着順
- バンク特性(先行有利度)と脚質の相性
- ステージ(予選/準決勝/決勝等)とグレード
確率は過大評価しがちなので、本命でも過信せず、実力差が小さい場合は確率を均し気味にしてください。
