...
def estimate_ai_win_probabilities(
    ...
    market_text = (
        "各選手のデータに含まれる\"market_win_prob_pct\"は、このレースの現在のオッズ"
        "(投票者全体の集合知)から逆算した、市場が織り込んでいる1着確率です"
        "(値が無い選手はオッズデータ未取得)。\n"
        "これは有力な参考情報ですが、絶対的な正解ではありません。市場は情報の少ない"
        "一般の投票者の資金も多く含むため、人気選手を過大評価し、地味だが実力のある"
        "選手を過小評価する傾向(いわゆる人気馬(選手)バイアス)が知られています。\n"
        "使い方の指針:\n"
        "  - あなた自身の判断(競走得点・脚質・展開等)による確率と、市場の確率が"
        "大きく異なる場合、まずその選手データを見直し、自分の判断根拠が本当に"
        "妥当か再検討してください。多くの場合、市場の方が多くの情報(当日の調子・"
        "気配等、あなたが見ていない情報)を織り込んでいる可能性があります\n"
        "  - それでも自分の判断根拠(データから明確に読み取れる材料)に自信がある"
        "場合は、市場と異なる確率をつけて構いません。市場確率をそのままコピーする"
        "必要はありません\n"
        "  - 特に、市場確率が低い(人気がない)選手に、データ上の理由なく市場を大きく"
        "上回る高い確率をつけることは避けてください\n"
    )
...
{lines_text}{bank_text}{stage_text}{weather_text}{development_text}{market_text}"is_local"がtrueの選手は、その開催地の地元選手であることを意味します。地元選手は声援を受けて
