...
最終更新: 2026-09-08(Grok) — 実績ゲートの設計違反を修正(券種・帯のプラン除外を削除)

### 直近の作業
- **問題:** UI/設計は「不調ステージ見送りのみ」なのに、race-planが
  不調券種・券種×オッズ帯・購入集合的中不足まで `is_recommended` を落としていた(再混入)。
- **修正:** `app/routers/ev.py` から上記3種のプラン除外を削除。
  残すのは (1) 不調ステージ(期待値<-50%) (2) 着順指定券種のステージサンプル不足 のみ。
- 第2段校正・高オッズ残差(確率への係数)は変更していない(禁止ではなく確率寄せ)。
- `schemas` のコメントと UI 文言を設計どおりに明記。

### 次にやること
1. 本修正をcommit/push→Renderデプロイ
2. デプロイ後、同じレースでプランを作り「不調券種除外」が理由に出ないことを確認
3. 必要なら少数replayで券種別の購入が戻るか確認(全件一括は不要)

### 注意
- 券種ゲートの**再混入禁止**。診断用途は `/purchases/diagnostics/*` に置く。
- 校正・EV閾値・選択ロジックの閾値は今回触っていない。
