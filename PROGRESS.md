...
最終更新: 2026-09-07(Claude) — 実績ゲート見送り合算を撤回(3連単誤除外事故)。
投票プラン4件の不具合対応(単一チケット集中・締切後表示・会場欠落を確認/一部修正)。

### 直近の作業
- **【重要・事故】** 券種/ステージ実績ゲートへの見送り(SkippedBet)合算を撤回。
  評価しただけの低確率候補まで仮想100円で平均に混ぜたため、実際は黒字の
  3連単が「実績-53.96%」という誤判定で全滅する事故が発生(2026-09-07)。
  `get_stage_expectancy_map`/`get_bet_type_expectancy_map`/
  `get_bet_type_odds_band_expectancy_map`は実購入のみに戻し、
  `CALIBRATION_SWITCH_AT`以降に絞る部分だけ維持。**見送りデータを実績判定に
  混ぜる設計は今後も避けること。**
...
- `/races/upcoming`の締切後表示の猶予を30分→5分に短縮
  (`overdue_min`パラメータ追加。締切を過ぎたレースが「投票する」表示のまま
  30分近く残っていた問題)。
- 未解決: 「本日7会場のうち2会場(いわき平・弥彦)が投票タブに出ない」
  「集計(/purchases/stats)が総ベット372件・2レース分しかない」の2点は
  コード上の原因が特定できず、DBの実データ確認待ち(下記「次にやること」参照)。

### 次にやること
1. **/purchases/stats の母集団が総ベット372件・2レースしかない件の原因調査。**
   `get_purchase_set_calibration_factors`(全期間・since制限なし)でも3連単が
   85件しかなく、想定される数千件规模と大きく乖離している。以下のどちらかを
   `since=all`と`since=calibration_switch`両方で比較して切り分ける。
   - Neon側のPurchaseテーブルの実際の行数が想定より少ない(データ消失/
     同期不整合/リプレイが実は反映されていない、等)
   - 集計クエリ側の条件が想定より絞り込みすぎている
   `git log`確認で`CALIBRATION_SWITCH_AT`自体は2026-09-04のままだったため、
   このコード変数が原因ではない。**DB側の実データを直接確認すること。**
2. 「いわき平・弥彦が投票タブに出ない」件も、スクレイピングでpost_time/entries
   が入っているか実データを確認してから対応する(コード上の除外リストは無い)。
3. ChatGPT分析9項目の項目2(本命予想→買い目確率→EVの変換過程の可視化)。
