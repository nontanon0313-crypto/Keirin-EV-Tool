...
最終更新: 2026-09-12(Grok) — サステナブル利益方針を確定。投票プランは的中率土台
...
- 本命1着はモデルEVがマイナスでも候補に残す（モデルEV不信頼のため）
- ポートフォリオ目的値も的中確率寄与優先
...
- ラインブースト倍率は使わない（1.0）
- 投票券種は三連単のみ
- `VOTING_CRITERIA_UPDATED_AT` = 2026-09-11 16:00:00 UTC

### 残作業
- 的中率土台の実購入で安定度（的中頻度）と収支を評価
- 必要ならオッズ下限のみ実績で調整（上限は付けない）
- モデルEVを主にした閾値運用には戻さない
