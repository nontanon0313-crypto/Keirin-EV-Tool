...
最終更新: 2026-09-05(Claude) — `gate-expectancy-raw`の実行結果で仮説を修正。2車単のbet_type_expectancy_pctは既に-85.9%(n=371)で0%カットラインを大きく下回っており、**全期間データに薄められている訳ではなかった**(前回の仮説は誤り)。実際の原因は`RETROACTIVE_CALIBRATION_CACHE_TTL_SECONDS=60分`のキャッシュ(「replay中に再計算しない」設計)。今回の24時間replayバッチ実行中、2車単のサンプルが50件を超えて悪化したのはこのバッチ自体によるものであり、バッチ実行中はキャッシュが固定されていたため、悪化を検知しないまま345件が積み上がった。現在は既に-85.9%が反映されているはずなので、`warm-calibration`で強制再計算した上で少数の追加replayを行い、2車単が今後は本当にゲートされるか確認するのが次のステップ。A級特選ステージ(-60.35%)もステージゲートのカットライン(-50%)を下回っており、こちらは正しく機能しているはず。オッズ帯別(300倍以上でROI乖離が集中)の原因切り分けにはodds-driftをまだ実行していない。
...
- 2車単 n=345 ratio4.42 actROI15.7%/predROI517.6%(壊滅的)。
  → `gate-expectancy-raw`で確認した全期間expectancy_pctは-85.9%(n=371)。
    ゲートの閾値(0%)は正しく下回っているが、60分キャッシュのため
    このバッチ実行中は反映されなかったと推測。要再検証。
- 300倍以上のオッズ帯(n=1812、母数最大)でROI乖離が集中(actROI1099%<predROI2460%)。
  的中率比は他帯と同程度なので、確率校正よりオッズドリフトを疑う。odds-drift未実行。

### 今やっていること / 次にやること
1. `warm-calibration`をPOSTしてbet_type/stage期待値キャッシュを強制再計算させた上で、
   少量の追加replay(20件程度)を行い、2車単の購入が今度は本当にゲートされ0件に
   近づくか確認する。ゲートされない場合はキャッシュ以外の原因(ロジック側の見落とし)
   を疑う。
2. `odds-drift`(since=calibration_switch)を実行し、300倍以上のオッズ帯で
   購入時オッズと最終/実現オッズのドリフトがROI乖離の主因かどうか確認する。
