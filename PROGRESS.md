...
最終更新: 2026-09-07(Grok) — 4.7-2 判定経路の分離検証(読み取り専用)を追加

### 直近の作業
- **ルール変更なし。** 高オッズ×高EVの利益は期待値上あり得る、という前提で分析のみ継続。
- **4.7-2実装:** `GET /purchases/diagnostics/decision-pipeline`
  - Stage1 本命車番 / Stage2 買い目確率(校正前後・購入/見送り) / Stage3 EV / Stage4 購入判定
  - bottleneck_hints と解釈メモ付き(高オッズ一律除外は根拠にならない旨を明記)
- 検証タブに「判定経路の分離検証」ボタンを追加
- 4.7-1 オッズ上限感度は実装済み。本番上限ルール化は**しない**(数値も高オッズ切ると悪化)

### 次にやること
1. 本変更をcommit/push→デプロイ後に decision-pipeline を実行
2. 4.7-3 補正係数の構造可視化
3. 閾値・購入ロジックの変更は分析結果を見てから設計合意
...
3. 補正係数0.8449の算出構造の可視化(条件別補正の要否判断材料を出す) — **次に着手する。**
