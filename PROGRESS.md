...
最終更新: 2026-09-05(Claude) — オッズ帯細分化(300-1000/1000-3000/3000以上)で再検証。**乖離は1000-3000倍に強く集中**していることが判明: n=495(サンプル十分)でratio5.78(的中率を5.8倍過大予測)・predROI2893%に対しactROI84%(約34倍の過大予測)。対照的に100-300倍・300-1000倍はほぼ健全(actROIがpredROIと同等かむしろ上回る)。3000倍以上もratio1.5・約5.8倍の過大予測だがn=121とやや少なめ。「乖離の主因はオッズドリフトではなく、1000-3000倍という特定の超大穴レンジで確率が体系的に過大」という具体的な仮説に絞れた。**設計判断が必要な段階**: このレンジを絞り込む/確率を下げる等の対応は選択ロジック変更にあたるため、のんと相談してから着手する。2車単は今回の24h窓でn=187・実績0件(actROI0%・predROI436%)だったが、これはwarm-calibration前の古い購入が混ざっている可能性が高く、直近のゲート状態は未確定(narrow window再確認が必要)。

### フェーズ
校正・ゲート改善は一段落。**様子見フェーズ。実資金投票はまだしない。**

### 直近の指標(2026-09-05, 24時間窓・last_n_races=300, n=3051)
- 全体ratio 1.36 / pred_ROI% 1579.9 / act_ROI% 835.5
- オッズ帯別(細分化後):
  - 100-300倍(n=1076): ratio1.28、**actROI611%>predROI504%で健全**
  - 300-1000倍(n=1174): ratio0.9、**actROI1423%>predROI1180%で健全(最良)**
  - **1000-3000倍(n=495): ratio5.78、actROI84%<predROI2893%(乖離の主因・要対応検討)**
  - 3000倍以上(n=121): ratio1.5、actROI2053%<predROI11967%(過大予測だがn少なめ)
  - 30-100倍(n=179): ratio2.89、actROI45%<predROI272%(こちらも過大)
- 2車単(n=187): 実績0件。warm-calibration前の残留データの可能性、要narrow window再確認。

### 今やっていること / 次にやること
1. **要のん判断**: 1000-3000倍レンジの確率が体系的に過大という結果が出た。
   対応案としては(a)このレンジだけ追加の確率縮小係数を設ける、
   (b)実績ゲートに券種×オッズ帯の粒度を持たせる、(c)様子見して
   サンプルをさらに積む、等が考えられるが、**選択ロジック・閾値の変更に
   あたるため、方針をのんと合意してから着手する。現時点では何も変更していない。**
2. 2車単のゲートが直近も機能しているか、狭い窓(直近replayバッチのみ)で
   再確認する(前回1時間20レース窓では0件を確認済みだが、今回の24h窓には
   混在があるため単発確認が必要)。
3. `predicted-vs-actual-return`のオッズ帯/EV帯/確率帯分解・odds-driftの
   構造的限界の注記は完了。

### 触ってはいけない・合意が必要なもの
- 選択ロジック(EV最大化 vs 的中率重視)の閾値は勝手に変えない
- 1000-3000倍レンジへの追加対応(確率縮小・ゲート追加等)も設計変更に該当。
  勝手に変えない。
- 実績ゲート・ステージゲートの集計窓(全期間 vs 直近)やキャッシュTTLを変えるのも
  設計変更に該当。勝手に変えない。
- run_day.shにwarm-calibration自動呼び出しを追加する案も、パイプラインの
  挙動変更にあたるため合意してから。
