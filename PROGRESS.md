...
最終更新: 2026-09-07(Grok) — 4.7-3 補正係数の構造可視化を追加

### 直近の作業
- **ルール・係数は変更していない。** 分析のみ。
- **4.7-3実装:** `GET /purchases/diagnostics/calibration-structure`
  - 第1段(遡及) / 第2段(購入残差)の overall・層別係数を一覧
  - 代表ケースの適用パス(どの層の係数が選ばれるか)を追跡
  - 1から大きく離れた層を notable として抽出
- 検証タブに「補正係数の構造」ボタン追加
- 4.7-1/2 は実装済み。高オッズ一律除外はしない方針を維持

### 次にやること
1. commit/push→デプロイ後に calibration-structure を実行して数値確認
2. 4.7-4 補正後p値・統計検定の追跡可能化
3. 係数や閾値の変更は、分析結果を見てから設計合意

### 注意
- calibration-structure は重い(第1段が全オッズ走査)。キャッシュ利用あり
- 勝手に係数を書き換えない
...
### 4.7 ChatGPT分析(2026-09-07)に基づく9項目の検証課題
ChatGPTによる実績分析(総ベット3,148件・的中37件・的中オッズ中央値344.7倍・
上位10的中で利益の64.2%等)を受けて、以下9項目を優先順位順に検証・実装する。
既存の予想ロジック・EV計算・選択閾値は勝手に変えず、まず読み取り専用の分析
機能を追加して現状を可視化してから、必要な修正を個別に判断する。

**着手前に確認した現状(2026-09-07時点)**
1. オッズ上限感度分析: **実装済み**(2026-09-07)。`/purchases/diagnostics/odds-cap-sensitivity`。本番ルール化は数値確認後に合意。
2. 本命車番→買い目確率→EV→購入判定の分離検証: **実装済み**(2026-09-07)。
   `/purchases/diagnostics/decision-pipeline`。car-pick等の既存指標も併用可。
3. 補正係数0.8449の構造検証: **実装済み**(2026-09-07)。`/purchases/diagnostics/calibration-structure`。
4. 補正後p値の検証: `calibration_significance`(二項検定)はあるが、
   入力値→補正前確率→…→p値を1系統で追跡する機能ではない。
5. 小サンプル条件の信頼区間: **未実装**。
6. バンク×券種×勝率帯×オッズ帯の交絡分析: **部分実装**
   (`combo_buckets`に該当の組み合わせなし)。
7. 同ライン×オッズ帯・勝率帯の交絡分析: **部分実装**
   (「券種×ライン絡み」はあるが該当の組み合わせなし)。
8. 黒字レース率・利益集中度・最大ドローダウン: **ほぼ未実装**
   (黒字レース率のみ`calibration_significance`内に存在)。
9. キャリブレーション改善と投資判断改善の分離: **未実装**。

**実装順序(合意済み)**
1. オッズ上限感度分析 — **実装済み**(読み取り専用)。本番閾値化は未着手。
2. 本命予想→買い目確率→EV→購入判定の分離検証 — **実装済み**。
3. 補正係数の算出構造の可視化 — **実装済み**。
4. 補正後p値・統計検定ロジックの追跡可能化 — **次に着手する。**
5. 小サンプル条件への信頼区間の追加
6. バンク別交絡分析(バンク×券種×勝率帯×オッズ帯)
7. 同ライン条件の交絡分析(ライン×オッズ帯・勝率帯)
8. 黒字レース率・利益集中度・最大ドローダウン・上位N件/オッズ帯除外分析
9. 「キャリブレーション改善」と「投資判断改善」の指標分離

**共通の制約(いずれの項目でも厳守)**
- 高オッズ・低オッズいずれについても「良い/悪い」と決めつけない。
- 小サンプルの高回収率を有効条件として即座に採用しない。
- バンク別・ライン別の数値だけを根拠に購入ルールを追加しない。
- 各項目は分析機能の追加(読み取り専用)を先に行い、購入ロジックの変更は
  分析結果を見てから個別に設計合意する。
