...
最終更新: 2026-09-05(Claude) — 24時間窓(last_n_races=300, n=3141, 209レース)で再検証。全体ratio 1.35・act_ROI 799%がpred_ROI 1538%の約半分に沈み、「予測期待値が実績より膨らんでいる」仮説をこちらの窓では支持。特に2車単がn=345でact_ROI 15.7%・predROI 517.6%(ratio 4.42)と壊滅的。この2車単は`bet_type_exp_map`(全期間実績収支率ベースの券種ゲート、カットライン0%)に引っかかっていない疑いが強い。原因はget_bet_type_expectancy_mapが確定済みPurchase全件(as-ofカット無し・全期間)を集計しており、古い実績に薄められて直近の悪化を検出できない可能性(4.2の未来リークと同根の問題)。確認用に読み取り専用の`GET /purchases/diagnostics/gate-expectancy-raw`を追加(実際のstage/bet_typeの実績収支率マップをそのまま返すだけ・閾値やロジックは未変更)。構文検証済み・未デプロイ。次はデプロイ後にこのエンドポイントを叩いて2車単の全期間expectancy_pctの実値を確認する。

### フェーズ
校正・ゲート改善は一段落。**様子見フェーズ。実資金投票はまだしない。**

### 直近の指標(2026-09-05, 24時間窓・last_n_races=300)
- n=3141, 209レース / 全体ratio **1.35** / pred_ROI% 1538.4 / act_ROI% 798.8
  (predがactの約1.9倍。乖離は実在すると見てよい)
- 券種別: 3連単 n=2796 ratio1.18 actROI927%/predROI1706%。
  **2車単 n=345 ratio4.42 actROI15.7%/predROI517.6%(壊滅的・要ゲート確認)**
- オッズ帯別: 100-300倍(n=1132)はactROI564%>predROI503%で健全。
  300倍以上(n=1812、最大母数)はactROI1099%<predROI2460%(乖離の主因)。
  ヒット率比(ratio)は100-300倍と300倍以上でほぼ同じ(1.22)なのに
  ROI乖離だけ300倍以上で顕著→的中率の校正ではなくオッズ側
  (odds-drift、購入時オッズと最終/実現オッズの差)を疑う。
- 勝率帯別: 0-5%(大穴)がn=3078で母数の98%。5-15%以上はまだサンプル不足。
- 6時間窓(n=570)では健全に見えたが、24時間窓で問題が顕在化。
  母数が小さいと結論を誤るため、判定は300件規模以上の窓で行うこと。

### 今やっていること / 次にやること
1. `gate-expectancy-raw`をデプロイ後に叩き、2車単の全期間expectancy_pctが
   実際に0%カットラインを超えているか確認する(全期間データが薄めている仮説の検証)。
   薄まっていると確認できたら、券種ゲートの集計窓をas-of/直近化する設計変更が
   必要になるが、**これは選択ロジックの閾値変更にあたるため、確認結果を見せた上で
   のんと設計合意してから着手する。**
2. 300倍以上のオッズ帯でROI乖離が集中している件は`odds-drift`エンドポイント
   (`app/routers/purchase_diagnostics.py`)で購入時オッズ vs 最終/実現オッズの
   ズレを確認する(まだ未実行)。
3. `predicted-vs-actual-return`のオッズ帯/EV帯/確率帯分解は完成・機能確認済み。
   新規エンドポイントの追加は不要。

### 触ってはいけない・合意が必要なもの
- 選択ロジック(EV最大化 vs 的中率重視)の閾値は勝手に変えない
- 実績ゲート・ステージゲートの集計窓(全期間 vs 直近)を変えるのも設計変更に該当。
  勝手に変えない。
...
- 2026-09-05: OddsParkの3連単/3連複パーサーについて、HTMLの空セルを含む位置対応の不具合を修正。実データで3連単3-7-1=14.0、3連複1-3-7=97.0を確認し、3連単210/210、3連複35/35で完全取得を確認。あわせて `scraper_import.py` のOdds保存処理を修正し、`is_complete=False` の券種では既存Oddsを削除せず、完全取得できた券種のみ `race_id + bet_type` 単位で洗い替える方式に変更。これにより、一部券種の取得失敗で他券種の既存Oddsまで消える問題を防止。
