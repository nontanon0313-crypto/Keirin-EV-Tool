...
最終更新: 2026-09-05(Claude) — `by_prob_band`追加後、直近窓(hours=6, last_n_races=100, n=570, 35レース)で検証実行。結果: ratio 0.74・act_ROI(1975%)がpred_ROI(1934%)を上回り、「予測期待値が実績より膨らんでいる」仮説はこの窓では否定的(むしろ実績の方がやや上)。ただし570件中557件が0-5%(大穴)帯・すべて3連単に偏っており、中〜本命帯(5-15%は13件のみ、15-30%以上は0件)はサンプル不足で判定不可。2車単はこの窓で0件(混入解消か単に出現しなかっただけか未確定)。全期間(n=3524)のratio 1.43は既知の通り古い購入混入のため参考外。次は中〜本命帯・3連単以外の券種のサンプルを増やすため追加replay→再検証。

### フェーズ
校正・ゲート改善は一段落。**様子見フェーズ。実資金投票はまだしない。**
追加の大量replayや係数いじりは急がなくてよい。

### 直近の指標(直近replay窓・`run_pva_recent.sh 6 100`基準・2026-09-05)
- n=570(35レース、すべて3連単) / pred的中% 1.56 / 実績的中% 2.11 / ratio **0.74**
- pred_ROI% 1934.8 / act_ROI% 1975.3(実績が予測をやや上回り健全)
- 勝率帯別: 0-5%(大穴)がn=557で大半を占め ratio 0.81・act_ROI 1602%。
  5-15%はn=13のみでノイズ大(ratio 0.4・act_ROI 5123% > pred_ROI 6455%は逆転)。
  15-30%・30%以上(本命)はこの窓でn=0のためまだ検証不可。
- オッズ帯別: 30-100倍(n=21) ratio 0.54 / 100-300倍(n=188) ratio 1.1 /
  300倍以上(n=361) ratio 0.63。300倍以上がact_ROI 2705%と支配的。
- 全体(全期間 n=3524)のratio 1.43は古い購入が支配するため校正判定には使わない。
  必ず`run_pva_recent.sh`の直近窓で判定すること。

### 今やっていること / 次にやること
1. 中〜本命帯(勝率帯15%以上)・3連単以外の券種のサンプルが直近窓でほぼ0件のため、
   `run_replay_continue.sh`で追加replayし、`run_pva_recent.sh`の時間窓を広げて
   (例: 24時間・last_n_races 300程度)これらの帯にサンプルが増えるか確認する。
2. 「予測期待払戻額 vs 実際払戻額の乖離」分解は`predicted-vs-actual-return`
   (by_odds_band・by_ev_band・by_prob_band)で完結できることを確認済み。
   新規エンドポイントは不要。
