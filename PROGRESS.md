...
最終更新: 2026-09-12(Claude) — 得点補正の二重適用を解消。band_factor/rank_blendどちらを採用するかは検証中

### 直近の確定（厳守）
- 投票は三連単のみ(ワイド・2車複・2車単・3連複は候補生成しない。見送り記録も不要
  ＝3連単の確定結果とオッズがあれば事後計算できるため)
- ラインブースト倍率は使わない（1.0）
- 1着: blended + 得点補正(具体的な方式は検証中。下記参照)
...
### 得点補正方式について(2026-09-12・検証中)
- band_factor(得点帯ごとの経験的倍率)とrank_blend(得点順位を重み0.55で直接合成)が
  同時に適用されており、二重適用になっていたことが判明。どちらを採用すべきかは
  「先にどちらを導入したか」という時期の記録が誤っていたため時期では判断できず、
  実際の精度で判断する方針(のん指示)。
- `/purchases/diagnostics/race-score-method-compare`を追加し、
  blendedのみ・band_factorのみ・rank_blend(複数の重み)のみ・旧二重適用、
  を同条件で比較できるようにした。この結果を見てから最終方式を確定する。
- 暫定措置として、二重適用を避けるためrank_blendのみを本番で使用中
  (診断結果次第でband_factorに戻す可能性あり)。

### 条件付き精度の目安（実際着順固定・246レース、得点補正方式確定前の数値）
- 1着約41.5% / 2着約50.8% / 3着約41.1% / 3連単Top1約6.5%
- (得点補正方式を確定させたら再検証)

### 残作業
- `race-score-method-compare`の結果を見て得点補正方式を最終確定
- Supabase制限解除後、必要なら再投票1回
- 新since以降の実購入でROI評価
