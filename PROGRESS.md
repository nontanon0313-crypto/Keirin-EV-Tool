...
最終更新: 2026-09-05(Grok) — 券種×帯ゲートが効かない原因修正(オッズ補完・min30)

### フェーズ
(b)券種×オッズ帯ゲート実装後、直近50Rで **3連単 ratio 0.97** と全体 ratio 1.1 まで改善。
ただし **1000-3000倍が64件・2車単67件** 残存。調査の結果 warm 上の帯セルは11件のみで薄い。

### 原因と修正
- 帯マップが `odds_at_purchase` のみ参照 → 欠損行が帯「不明」で落ち、ゲート用セルが不足
- **修正**: final_odds / 的中時の実現倍率で補完。帯ゲート min_samples **50→30**
- warm-calibration 応答に `bet_type_expectancy` / `bet_type_odds_band_expectancy` /
  `bet_type_odds_band_negative` を含め、すり抜けの有無を数値で確認可能に

### 次にやること
1. デプロイ → warm → 負の帯一覧を確認（3連単|1000-3000 等が入るか）
2. `run_replay_continue.sh 50 2` → `run_pva_recent.sh 3 50`
3. 1000-3000と2車単のnが落ちるか確認

### 触ってはいけないもの
- 選択ロジックの根本変更、line_boost、集計窓変更は合意してから
