...
最終更新: 2026-09-06(Grok) — 集計「購入履歴なし」のUTC境界バグを修正

### バグ
`/purchases/stats` 既定 `since=calibration_switch` が
`datetime(2026,9,6,0,0,0)` (UTC解釈) のため、
JST 9/6 0:00〜9:00 の再投票(purchased_at は UTC 9/5)が集計から消えていた。

### 修正
- `CALIBRATION_SWITCH_AT` → `2026-09-05 15:00:00` UTC (= JST 9/6 00:00)
- 空レスポンスに since_resolved / settled_count_all_time を付与

### 確認
デプロイ後に検証タブ「集計を表示」。
それでも空なら `GET /purchases/stats?since=all` で全期間確認。
