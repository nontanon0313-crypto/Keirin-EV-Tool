...
最終更新: 2026-09-05(Grok) — **方針B実装: 高オッズ帯(1000-3000/3000+)の的中率残差で確率縮小**

### フェーズ
的中率の直近校正は良好(ratio≒1)。課題は1000-3000倍帯の勝率過大と想定ROI膨張。
全期間ROIがプラスのため実績ゲートでは切れない → **確率を実績的中に合わせて縮める(B)** を実装。

### 今回の実装(B)
- `get_high_odds_residual_factors`: 1000-3000倍・3000倍以上について
  予測的中率 vs 実績的中率の比率で係数を算出（券種×帯優先、なければ帯）
- `race-plan` で購入集合残差の**後に**係数を掛ける（足切り・禁止ではない）
- 下限0.08、minサンプル 帯30 / 券種×帯20
- warm-calibration に `high_odds_residual` を出力

### 次にやること
1. デプロイ → warm で high_odds_residual の係数を確認
2. `run_replay_continue.sh 50 2` → `run_pva_recent.sh 3 50`
3. 確認: 1000-3000の predROI が下がるか、全体 ratio が悪化しないか、
   的中率の一致は維持されるか
