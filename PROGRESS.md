...
最終更新: 2026-09-05(Claude) — 2件確認。①2車単ゲート問題は解決を確認: warm-calibration実行後の直近窓(1時間・20レース・n=236)で2車単の購入が0件になり、キャッシュ更新で正しくゲートされることを確認した。②odds-driftを実行した結果、全券種・全EV帯でdrift_pctが一律0.0という結果になったが、これはバグではなく構造的な限界と判明: 本ツールはオッズを結果確定後に1回だけスクレイピングする設計(`races.py`のconfirm-result処理でfinal_oddsもodds_at_purchaseと同じOddsスナップショットから埋めている)ため、締切前後の実際のオッズ変動はこのデータでは原理的に測定できない。odds-driftのdocstring/noteにその旨明記した。300倍以上のオッズ帯でのROI乖離の原因はオッズドリフトでは説明できないため保留に戻し、より細かいオッズ帯分割(300-1000/1000-3000/3000以上)を`predicted-vs-actual-return`に追加し、乖離が超高倍率側に集中しているか確認できるようにした。構文検証済み・未デプロイ。

### フェーズ
校正・ゲート改善は一段落。**様子見フェーズ。実資金投票はまだしない。**

### 直近の指標(2026-09-05, warm-calibration後の直近窓・hours=1 last_n_races=20)
- n=236, 15レース、すべて3連単(2車単は0件=ゲート確認できた)
- ratio 0.87 / pred_ROI% 1002.1 / act_ROI% 755.8(小サンプルにつき参考値)
- オッズ帯別: 300倍以上 n=146 ratio1.8 actROI320%/predROI1318%(乖離大、要継続観察)。
  ただしこの窓はn=15レースと小さいため断定はまだ早い。

### 今やっていること / 次にやること
1. `run_replay_continue.sh`で追加replay→`run_pva_recent.sh`実行を繰り返し、
   細分化した300-1000/1000-3000/3000倍以上の各帯にサンプルが貯まってから
   乖離がどの倍率帯に集中しているか判定する。
2. 運用改善案(要のん確認・設計合意): `warm-calibration`をrun_day.shの冒頭で
   毎回自動的に呼ぶようにすれば、今回のような「券種ゲートが直近の悪化を
   検知するまでに60分のキャッシュタイムラグが生じる」問題を予防できる。
   **まだ実装していない。のんの合意を得てから着手する。**
