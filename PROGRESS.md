...
最終更新: 2026-09-05(Grok) — **(b) 実績ゲートを券種×オッズ帯に細分化**

### フェーズ
校正・ゲート改善を継続。**実資金投票はまだしない。**
1000-3000倍の体系的過大予測への対応として、(b)を実装した。

### 今回の実装(b)
- `get_bet_type_odds_band_expectancy_map`: 券種×オッズ帯の実績収支率
  （帯: 1-5 / 5-10 / 10-30 / 30-100 / 100-300 / 300-1000 / **1000-3000** / 3000以上）
- `race-plan` の実績ゲートに追加: サンプル≥50 かつ expectancy&lt;0% の
  **券種×帯**を見送り（例: `不調券種×オッズ帯除外(3連単/1000-3000倍:...)`）
- 券種まるごとゲートは維持（2車単など全体赤字は従来どおり）
- `warm-calibration` でも当該マップをウォーム
- 購入集合残差のオッズ帯ラベルも同じ細分化に揃え

### 次にやること
1. デプロイ後: `warm-calibration` → `run_replay_continue.sh 50 2` →
   `run_pva_recent.sh 3 50`
2. 確認: 1000-3000倍の購入が減る/消えるか、100-300・300-1000は残るか、
   直近 ratio が悪化しないか
3. 2車単が狭い窓で0件か再確認

### 触ってはいけない・合意が必要なもの
- 選択ロジックの根本変更、line_boost、集計窓(全期間vs直近)の変更は合意してから
- 閾値の勝手な追加足切りはしない（今回は既存0%を帯に適用しただけ）
