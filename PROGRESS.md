...
最終更新: 2026-09-07(Grok) — 4.7-1 オッズ上限感度分析(読み取り専用)を追加

### 直近の作業（2026-09-07）
- **ChatGPT分析9項目の1番目を実装:** `GET /purchases/diagnostics/odds-cap-sensitivity`
  - 仮想上限 50/100/200/300/500/1000 倍で既存Purchaseを事後フィルタし、件数・的中率・ROI・損益・除外側損益を比較
  - 券種別の内訳、利益上位10件の集中度も返す
  - **購入ロジック・閾値・校正は変更しない**(読み取り専用)
- 検証タブに「オッズ上限の感度分析」ボタンを追加
- `python3 -m py_compile app/routers/purchase_diagnostics.py` 成功

### 次にやること
1. 本変更をcommit/push→デプロイ後、感度分析を実行して数値を確認
2. 数値を見てから「上限を本番ルールに入れるか」を設計合意(勝手に閾値変更しない)
3. 4.7の2番目: 本命予想→買い目確率→EV→購入判定の分離検証

### 注意
- n不足(既定30未満)のシナリオは採用判断に使わない
- 高オッズ/低オッズを良し悪しと決めつけない
...
2. 本命車番→買い目確率→EV→購入判定の分離検証: **部分実装**
   (`purchases.py`に`ai_pick_top3_rate_pct`等はあるが、確率変換の経路を
   1本で追跡できる機能ではない)。
3. 補正係数0.8449の構造検証: 係数算出の仕組み自体(`_apply_calibration`等)は
   あるが、条件別補正の要否を判断できる整理された出力はない。
...
2. 本命予想→買い目確率→EV→購入判定の分離検証 — **次に着手する。**
3. 補正係数0.8449の算出構造の可視化(条件別補正の要否判断材料を出す)
4. 補正後p値・統計検定ロジックの追跡可能化
5. 小サンプル条件への信頼区間の追加
